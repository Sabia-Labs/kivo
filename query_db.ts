import { db } from "./apps/kivo-api/src/db/client";
import { tasks, requests, teamCapabilities } from "./apps/kivo-api/src/db/schema";
...
async function run() {
  ...
  const [cap] = await db.select().from(teamCapabilities).where(eq(teamCapabilities.teamId, task.teamId)); // wait, I should get the cap id
  console.log("Wait, I need the specific capability. Let me just print the first team capability with foreach type.");
