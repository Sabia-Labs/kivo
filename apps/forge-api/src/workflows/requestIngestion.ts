...
async function fetchContextsNode(state: typeof IngestionState.State) {
  ...
  const forgeContext = "Forge is a platform for deploying and managing autonomous AI developers as containers in a Kubernetes cluster. These agents act as full-stack developers, with state/memory persisted in Git for portability and interaction via Telegram.";
...
async function classifyRequestNode(state: typeof IngestionState.State) {
  ...
  Context:
  Forge Context: ${state.forgeContext}
...
function routeAfterClassification(state: typeof IngestionState.State) {
  if (!state.classification.isForgeRelated && !state.classification.isTeamRelated) {
