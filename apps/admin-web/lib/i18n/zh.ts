import { Dictionary } from "./en";

export const zh: Dictionary = {
  brand: "灵巢",
  nav: {
    about: "关于我们",
    contactSales: "联系销售",
    pricing: "产品定价",
    login: "登录",
    getStarted: "立即开始",
    setup: "团队配置",
    agents: "智能体",
    logout: "退出登录",
  },
  hero: {
    badge: "AI 智能体团队，以最佳实践管理",
    headline: "按需提供团队卓越表现。",
    subheadline: "灵巢 以世界级的组织管理纪律部署自主 AI 智能体团队 —— 无论您从事哪个领域，都能让您的智能体团队像管理良好的人类组织一样交付成果。",
    ctaPrimary: "开始使用",
    ctaSecondary: "联系销售",
  },
  stats: {
    items: [
      { label: "交付周期加速", value: "10×" },
      { label: "操作可审计性", value: "100%" },
      { label: "自主运行", value: "24/7" },
      { label: "上下文零丢失", value: "0" },
    ],
  },
  governance: {
    sectionBadge: "结构化自主",
    sectionTitle: "全方位的控制与可见性。",
    sectionSubtitle:
      "灵巢 用受治理、结构化、可量化的执行层取代了随机的聊天机器人交互。您的团队自主运行，但始终处于监督之下。",
    oldWayTitle: "通用 AI",
    kivoWayTitle: "灵巢 平台",
    items: [
      {
        old: "不可预测的输出和幻觉",
        kivo: "严格的基于角色的工作流和护栏",
      },
      {
        old: "没有记忆的孤立对话会话",
        kivo: "基于 Git 的跨任务持久化记忆",
      },
      {
        old: "黑盒执行",
        kivo: "实时可观测性和健康评分",
      },
      {
        old: "智能体采取未经批准的操作",
        kivo: "多级人工参与的审批流程",
      },
    ],
  },
  howItWorks: {
    sectionBadge: "执行周期",
    sectionTitle: "灵巢 如何编排工作。",
    steps: [
      {
        number: "01",
        title: "创建您的 AI 团队",
        description: "部署一个具有明确角色和所有权的角色化智能体小队。",
      },
      {
        number: "02",
        title: "分配工作",
        description: "分配复杂的任务目标。团队负责人将任务委派给专业智能体。",
      },
      {
        number: "03",
        title: "监控与审批",
        description: "审查进度，批准关键行动，并持续追踪团队健康状况。",
      },
    ],
  },
  pillars: {
    sectionBadge: "核心基础设施",
    sectionTitle: "专为运营纪律而建。",
    sectionSubtitle:
      "灵巢 提供必要的基础设施，将 AI 智能体视为负责任的团队成员，而不仅仅是工具。",
    items: [
      {
        title: "工作流图谱",
        description: "视觉化执行映射确保任务遵循定义的顺序和依赖关系。",
        icon: "GitMerge",
      },
      {
        title: "持久化记忆",
        description: "每一个决策、上下文和输出都经过安全版本化并存储。",
        icon: "Activity",
      },
      {
        title: "审批系统",
        description: "对智能体在未经人工授权下可执行的操作进行细粒度控制。",
        icon: "ShieldCheck",
      },
      {
        title: "团队健康评分",
        description: "实时遥测智能体速度、阻塞项和整体运营效率。",
        icon: "TrendingUp",
      },
    ],
  },
  templates: {
    sectionBadge: "基础设施模板",
    sectionTitle: "可部署的团队基础设施。",
    sectionSubtitle: "立即部署针对您的领域预先配置了正确角色、提示词和工作流的专业 AI 团队。",
  },
  controlPlane: {
    sectionBadge: "控制平面",
    sectionTitle: "您掌控团队表现的驾驶舱。",
    sectionSubtitle:
      "灵巢 控制平面为领导者提供了一个单一的界面，用于配置智能体团队、设置护栏、监控健康状况，并保持对您的 AI 劳动力的完全控制权。",
    features: [
      "按智能体和操作类型设置多级审批规则",
      "实时团队健康评分与活动日志",
      "按团队和任务的成本归归因",
      "符合您运营规范的工作流模板",
      "即时回滚与智能体替换",
      "持久记忆 —— 团队始终可从 Git 恢复",
    ],
  },
  cta: {
    headline: "立即扩展您的执行力。",
    subheadline: "在几分钟内部署您的第一个自主 AI 团队，体验结构化的自主权。",
    ctaPrimary: "部署您的团队",
    ctaSecondary: "联系销售",
  },
  footer: {
    tagline: "结构。治理. 交付。",
    product: "产品",
    company: "公司",
    legal: "法律",
    links: {
      features: "功能",
      pricing: "价格",
      docs: "文档",
      changelog: "更新日志",
      about: "关于我们",
      blog: "博客",
      careers: "招贤纳士",
      contact: "联系我们",
      privacy: "隐私政策",
      terms: "服务条款",
    },
    copyright: "© 2026 灵巢。保留所有权利。",
  },
  login: {
    title: "欢迎回来",
    subtitle: "登录您的 灵巢 账户",
    emailLabel: "电子邮件",
    emailPlaceholder: "you@company.com",
    passwordLabel: "密码",
    passwordPlaceholder: "········",
    signIn: "登录",
    continueWithEmail: "通过邮箱继续",
    orContinueWith: "或通过以下方式继续",
    google: "通过 Google 继续",
    microsoft: "通过 Microsoft 继续",
    noAccount: "还没有账户？",
    signUp: "立即注册",
    ssoComingSoon: "SSO 即将上线",
    checkEmail: "查收您的邮件",
    sentCode: "我们发送了一个4位验证码至",
    verifyCode: "验证验证码",
    back: "返回",
    invalidCode: "验证码无效",
    failedSendCode: "验证码发送失败",
    welcomeBack: "欢迎回来！",
    devLogin: "快速开发登录",
  },
  signup: {
    title: "组建您的 AI 智能体团队",
    subtitle: "选择模板，数分钟内完成部署。",
    steps: {
      account: "账户",
      workspace: "工作空间",
      team: "您的团队",
    },
    step1: {
      title: "创建您的账户",
      nameLabel: "全名",
      namePlaceholder: "张伟",
      emailLabel: "工作邮箱",
      emailPlaceholder: "zhang@company.com",
      passwordLabel: "密码",
      passwordPlaceholder: "至少8个字符",
      next: "继续",
      orSignUpWith: "或通过以下方式注册",
      termsText: "点击继续即表示您同意我们的",
      termsLink: "服务条款",
      privacyLink: "隐私政策",
      and: "和",
    },
    step2: {
      title: "为您的工作空间命名",
      workspaceLabel: "工作空间名称",
      workspacePlaceholder: "示例公司",
      next: "继续",
      back: "返回",
      taken: "该工作空间已被占用。",
    },
    step3: {
      title: "选择您的团队模板",
      subtitle: "选择起始结构。您随时可以在此基础上添加更多智能体。",
      next: "继续",
      back: "返回",
    },
    step4Starter: {
      title: "配置您的 灵巢 Starter 团队",
      subtitle: "为您的团队命名并指定一位领导者。",
      teamNameLabel: "团队名称",
      teamNamePlaceholder: "例如：运营、研究、战略…",
      teamPrefixLabel: "团队缩写 (ID前缀)",
      teamPrefixPlaceholder: "例如：OPS、RES、STR…",
      teamPrefixHint: "用于任务 ID (例如：OPS-1, OPS-2)。",
      launch: "创建团队",
      back: "返回",
    },
    step4Engineering: {
      title: "配置您的工程小队",
      subtitle: "每个工程团队固定包含一名组长。请在下方添加工程师、架构师和经理。",
      roles: {
        engineer: {
          title: "软件工程师",
          description: "实现功能、编写测试、按照您的 SDLC 提交 PR。",
        },
        architect: {
          title: "软件架构师",
          description: "设计系统、审查架构决策、执行技术标准。",
        },
        pm: {
          title: "产品经理",
          description: "管理待办事项、撰写工单、定义验收标准、跟踪进度。",
        },
      },
      pmFixedBadge: "必须保留",
      kivoPmTitle: "灵巢 组长",
      kivoPmHint: "您团队的协调者和负责人。每个团队有且仅有一名组长。",
      agentNamePlaceholder: "智能体名称",
      launch: "创建团队",
      back: "返回",
      agentsLabel: "个智能体",
    },
    haveAccount: "已有账户？",
    signIn: "立即登录",
    ssoComingSoon: "SSO 即将上线",
    creatingAccount: "正在创建团队…",
    defaultTeamLeadName: "张经理",
  },
  setup: {
    title: "团队配置",
    subtitle: "完成团队配置以开始部署智能体。",
    progressLabel: "配置进度",
    section1: {
      title: "团队基本信息",
      teamNameLabel: "团队名称",
      teamNamePlaceholder: "工程小队 Alpha",
      missionLabel: "使命",
      missionPlaceholder: "以卓越工程交付产品核心平台功能。",
      agentsTitle: "您的智能体",
      pmFixedBadge: "必须保留",
      agentNameLabel: "名称",
      agentRoleLabel: "角色",
      addAgent: "添加智能体",
      removeAgent: "移除",
    },
    createTeam: "创建团队",
    saveChanges: "保存修改",
    creating: "正在创建团队…",
    successTitle: "团队已创建！",
    successSubtitle: "您的智能体团队已就绪。前往团队仪表盘开始使用。",
  },
  agents: {
    title: "您的团队",
    subtitle: "配置每个智能体并将其连接到 Telegram。",
    teamReady: "团队就绪",
    teamWarning: "需要操作",
    teamCritical: "团队离线",
    teamReadyHint: "所有智能体均在线且已完成配置。",
    teamWarningHint: "部分智能体需要关注。",
    teamCriticalHint: "一个或多个智能体处于离线状态。",
    sendMessage: "发送消息",
    editName: "编辑名称",
    saveName: "保存",
    cancelEdit: "取消",
    changeAvatar: "更换头像",
    healthOnline: "在线",
    healthStarting: "启动中",
    healthOffline: "离线",
    roleLabels: {
      team_lead: "团队负责人",
      software_engineer: "软件工程师",
      software_architect: "软件架构师",
      product_manager: "产品经理",
      executive_assistant: "行政助理",
      executive_operations_lead: "执行运营主管",
      support_lead: "客服主管",
      support_responder: "客服专员",
      support_analyst: "客服分析师",
      marketing_lead: "市场主管",
      content_specialist: "内容专家",
      growth_specialist: "增长专家",
      sales_manager: "销售经理",
      account_executive: "客户经理",
      business_operations_analyst: "业务运营分析师",
      business_developer: "业务开发",
      finance_manager: "财务经理",
      legal_counsel: "法律顾问",
    },
    roleDescriptions: {
      team_lead: "协调团队并确保交付质量。",
      software_engineer: "构建高质量功能并解决技术漏洞。",
      software_architect: "定义模式并确保结构完整性。",
      product_manager: "产品目标和待办事项健康的守护者。",
      executive_assistant: "Operator 生产力的全能合作伙伴。",
      executive_operations_lead: "协调执行优先级、决策、后续跟进和运营节奏。",
      support_lead: "确保客户满意度和 SLA 合规性。",
      support_responder: "以同理心和速度直接协助用户。",
      support_analyst: "分析趋势并记录知识。",
      marketing_lead: "战略品牌守护者和需求生成器。",
      content_specialist: "创作引人入胜的故事和品牌内容。",
      growth_specialist: "优化渠道并运行增长实验。",
      sales_manager: "推动营收增长和销售战略。",
      account_executive: "管理复杂交易并达成合作伙伴关系。",
      business_operations_analyst: "分析运营信号、跟踪 OKR，并准备投资者或董事会就绪的更新。",
      business_developer: "识别并限定新的机会。",
      finance_manager: "财务诚信和效率的守护者。",
      legal_counsel: "通过法律卓越保护公司。",
    },
    roleSuggestedNames: {
      team_lead: "亚历克斯",
      software_engineer: "乔丹",
      software_architect: "凯西",
      product_manager: "萨姆",
      executive_assistant: "詹姆斯",
      executive_operations_lead: "凯西",
      support_lead: "罗宾",
      support_responder: "泰勒",
      support_analyst: "摩根",
      marketing_lead: "斯凯勒",
      content_specialist: "艾弗里",
      growth_specialist: "莱利",
      sales_manager: "昆恩",
      account_executive: "杰米",
      business_operations_analyst: "摩根",
      business_developer: "布莱克",
      finance_manager: "帕特",
      legal_counsel: "德鲁",
    },
    telegram: {
      sectionTitle: "Telegram",
      notConfigured: "未配置",
      configured: "已配置",
      step1Title: "创建 Telegram 机器人",
      step1Hint: "打开 Telegram，向 @BotFather 发送消息，输入 /newbot，按步骤操作后将返回的 Token 粘贴到下方。",
      botTokenLabel: "机器人 Token",
      botTokenPlaceholder: "123456789:AAF…",
      botTokenSave: "连接机器人",
      registering: "正在注册机器人…",
      step2Title: "机器人注册成功！",
      step2Hint: "您的机器人已在 Telegram 上激活。请输入配对码以关联此智能体。",
      pairingCodeLabel: "配对码",
      pairingCodePlaceholder: "6位配对码",
      pairingCodeSave: "配对智能体",
      complete: "Telegram 配置完成",
      changeToken: "更换 Token",
    },
  },
  langSwitcher: {
    label: "语言",
  },
  agentPage: {
    backToSquad: "返回团队",
    openHistory: "对话历史",
    openBrain: "大脑",
    openDashboard: "仪表盘",
    openTools: "工具",
    openSecurity: "安全",
    notFound: "未找到该智能体",
    chat: {
      placeholder: "向智能体发送消息…",
      send: "发送",
      emptyTitle: "开始对话",
      emptySubtitle: "您与该智能体的消息将显示在这里。",
      you: "您",
      thinking: "思考中…",
    },
    history: {
      title: "对话历史",
      empty: "暂无对话记录。",
      from: "来自",
      viewAgent: "查看智能体",
    },
    brain: {
      title: "智能体大脑",
      subtitle: "定义该智能体的核心个性与记忆。",
      personality: "个性",
      personalityPlaceholder: "描述智能体的语气、沟通风格和性格特点…",
      identity: "身份与角色",
      identityPlaceholder: "定义智能体在团队中的角色、专注领域和定位…",
      longTermMemory: "长期记忆",
      longTermMemoryPlaceholder: "智能体应始终记住的关键上下文…",
      save: "保存",
      saving: "保存中…",
      saved: "大脑已更新！",
    },
    model: {
      title: "大语言模型",
      change: "更换",
      cancel: "取消",
      save: "保存",
      saved: "模型已更新！",
    },
  },
  agentDashboard: {
    backToAgent: "返回智能体",
    currentTask: "当前任务",
    noCurrentTask: "暂无活跃任务。",
    recentTasks: "最近任务",
    noRecentTasks: "暂无已完成任务。",
    seeMore: "查看更多",
    dailyContributions: "每日任务贡献",
    tokenTimeline: "Token 消耗",
    tasks: "任务",
    tokens: "tokens",
  },
  teamsPage: {
    title: "我的团队",
    subtitle: "您所有的智能体团队，尽在一处。",
    agents: "智能体",
    noAgents: "暂无智能体。",
    noTeams: "暂无团队。",
    viewAgent: "打开智能体",
    newTeam: "新建团队",
    newTeamComingSoon: "新建团队功能即将上线。",
    workspaceLabel: "工作区",
  },
  teams: {
    types: {
      starter: {
        name: "灵巢 Starter",
        description: "配备一名行政助理，助您轻松开启。简单且灵活。",
      },
      "product-delivery": {
        name: "产品交付",
        description: "具备 SDLC 纪律的完整软件交付团队。",
      },
      "customer-support": {
        name: "客户支持",
        description: "为您的用户提供自动化的支持团队。",
      },
      sales: {
        name: "销售与营收",
        description: "推动增长并管理您的销售流水线。",
      },
      marketing: {
        name: "市场与增长",
        description: "建立您的品牌并生成需求。",
      },
      finance: {
        name: "财务",
        description: "管理您的预算和财务健康。",
      },
      legal: {
        name: "法律",
        description: "保护您的业务并确保合规。",
      },
      "executive-operations": {
        name: "执行运营",
        description: "协调优先级，跟踪 OKR，并保持执行节奏。",
      },
    },
  },
  capabilities: {
    product_delivery: {
      triage: { name: "整理待办事项", desc: "组织并精炼项目待办事项。" },
      create_story: { name: "创建用户故事", desc: "将需求转化为详细的用户故事。" },
      implement: { name: "实现任务", desc: "执行开发并开启 Pull Request。" },
      review: { name: "代码审查", desc: "审查 PR 的质量和标准。" },
      tech_debt: { name: "技术债评估", desc: "识别并记录技术债务。" },
      troubleshoot: { name: "排查漏洞", desc: "识别报告漏洞的根源。" },
      test_coverage: { name: "提高测试覆盖率", desc: "为特定系统区域实现测试。" },
      delivery_flow: { name: "功能交付周期", desc: "从故事到审查的端到端流程。" },
      open_pr: { name: "开启 Pull Request", desc: "为当前任务创建 PR。" },
      summary: { name: "周期总结", desc: "生成当前工作周期的总结报告。" },
    },
    customer_support: {
      answer: { name: "起草客户工单回复", desc: "起草针对客户问询的有帮助且富同理心的回复。" },
      escalate: { name: "上报漏洞", desc: "将工单转化为工程漏洞。" },
      gather: { name: "收集支持信息", desc: "收集并综合相关的文档 and 页面。" },
      read: { name: "读取客户工单", desc: "分析和总结客户支持工单。" },
      resolution_flow: { name: "客户支持解决流程", desc: "用于获取、研究、起草和提交客户工单回复的端到端流程。" },
      send_answer: { name: "发送客户回复", desc: "将回复写入外部工单系统。" },
      triage: { name: "分拣队列", desc: "优先排序并分配待处理的客服工单。" },
      write_kb: { name: "编写知识库文章", desc: "根据常见问题创建文档。" },
    },
    sales: {
      research: { name: "研究潜在客户", desc: "分析潜在客户的价值匹配度。" },
      proposal: { name: "准备提案", desc: "创建定制化的业务提案。" },
    },
    marketing: {
      ideation: { name: "内容策划", desc: "生成高影响力的内容创意。" },
      copy: { name: "撰写草案", desc: "创建以转化为中心的内容草案。" },
    },
    finance: {
      audit: { name: "费用审计", desc: "识别财务异常或浪费。" },
    },
    legal: {
      review: { name: "合同审查", desc: "分析协议的风险和保护。" },
    },
    executive_operations: {
      draft_executive_briefing: { name: "起草执行简报", desc: "为会议或战略讨论创建简洁的简报。" },
      prepare_board_update: { name: "准备董事会更新", desc: "使用公司指标、优先级和决策准备结构化的董事会更新。" },
      prepare_investor_update: { name: "准备投资者更新", desc: "准备清晰的投资者更新，总结进展和关键指标。" },
      run_okr_check_in: { name: "运行 OKR 检查会", desc: "审查 OKR、进度、阻碍因素并确定所需的决策。" },
      summarize_leadership_meeting: { name: "总结领导力会议", desc: "创建执行摘要，包括决策、行动项、所有者和截止日期。" },
      track_action_items: { name: "追踪执行行动项", desc: "审查公开的行动项，识别阻碍因素并建议后续行动。" },
    },
  },
};
