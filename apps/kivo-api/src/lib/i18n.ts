...
const translations: Record<TranslationKey, { en: string; pt: string; de: string; zh: string }> = {
  integrationMissing: {
    ...
    de: "Für diese Aufgabe wird das Tool '{tool}' benötigt, aber die zugehörige Integration ist für dieses Team nicht konfiguriert. Bitte verbinden Sie die Integration in den Teameinstellungen.",
  ...
  taskCompletedSuccess: {
    ...
    de: "Aufgabe '{title}' wurde erfolgreich abgeschlossen. Ergebnis: {result}",
    zh: "任务 '{title}' 已成功完成。结果：{result}"
  },
  taskCompletedSuccessDefaultResult: {
    en: "No result provided.",
    pt: "Nenhum resultado fornecido.",
    de: "Kein Ergebnis angegeben.",
    zh: "未提供结果。"
  },
  requestFailed: {
    en: "Request Failed",
  ...
  operatorRetryComment: {
    ...
    de: "Der Operator hat einen neuen Versuch angefordert. Zusätzliche Anweisungen wurden angewendet: {instructions}",
  ...
  retryAgentReply: {
    ...
    de: "Ich habe Ihre neuen Anweisungen erhalten und werde den Schritt **{title}** sofort erneut ausführen!\n\n**Zusätzliche Anweisungen:**\n{instructions}",
  ...
  retryRequestStateEntry: {
    ...
    de: "Der Operator hat einen neuen Versuch für die fehlgeschlagene Aufgabe '{title}' angefordert. Kommentar: \"{comment}\". Angewendete zusammengefasste Anweisungen: \"{instructions}\"",
  ...
  bypassTaskResult: {
    ...
    de: "Auf Anweisung des Operators übersprungen.",
    zh: "已由人工操作员指令跳过。"
  },
  bypassAgentReply: {
    ...
    de: "Verstanden. Der fehlgeschlagene Schritt **{title}** wird übersprungen und der Workflow mit der nächsten Aufgabe fortgesetzt.",
  ...
  bypassRequestStateEntry: {
    ...
    de: "Der Operator hat das Überspringen der fehlgeschlagenen Aufgabe '{title}' angefordert. Kommentar: \"{comment}\". Die Aufgabe wurde auf Anweisung des Operators übersprungen.",
  ...
  cancelRequestStateEntry: {
    ...
    de: "Der Operator hat den Abbruch des Anfrage-Workflows bei Aufgabe '{title}' angefordert. Kommentar: \"{comment}\".",
  ...
  cancelAgentReply: {
    ...
    de: "Verstanden. Ich habe den Anfrage-Workflow wie gewünscht abgebrochen.",
    zh: "明白。我已根据要求取消了请求工作流。"
  },
  workflowExecutionError: {
    en: "Error executing task: {reason}",
    pt: "Erro ao executar a tarefa: {reason}",
    de: "Fehler bei der Ausführung der Aufgabe: {reason}",
    zh: "执行任务时出错：{reason}"
  },
  otherTeamRoutingResponse: {
    ...
    de: "Diese Anfrage passt besser zu einem anderen Team: {otherTeam}. Bitte richten Sie Ihre Anfrage an dieses Team.",
 ...
 */
export function t(key: TranslationKey, lang: "pt" | "en" | "de" | "zh", params?: Record<string, string>): string {
 ...
 */
export async function resolveWorkspaceLanguage(teamId: string): Promise<"en" | "pt" | "de" | "zh"> {
  try {
    ...
    if (teamRecord?.workspaceId) {
      ...
      if (
        workspaceRecord?.language === "pt" ||
        workspaceRecord?.language === "de" ||
        workspaceRecord?.language === "zh"
      ) {
